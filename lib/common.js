...
common.getPlaceSelectWhatQuery = () => {
    let query = "SELECT pTab.*,";

    // Start Most Picks
    // 해당 플레이스가 갖고있는 픽들 like + comment순 (인기순) 전부 가져오기
    query += " (SELECT ";
    query += " GROUP_CONCAT(";
    ...
    query += " WHERE piTab.pi_p_id = pTab.p_id) AS pMostPicks,";
    // End Most Picks

    // 좋아요 여부
    query += " IF(mlpLikeTab.cnt > 0, 'Y', 'N') AS pIsLike,";

    // place like count
    query += " IFNULL(mlpCntTab.cnt, 0) AS pLikeCnt,";

    // place comment count
    query += " IFNULL(mcpCntTab.cnt, 0) AS pCommentCnt,";
