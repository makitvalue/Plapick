...
router.get('', async (req, res) => {
    try {
        ...
        // 특정 사용자가 게시한 게시물 / 특정 플레이스에 게시된 게시물 / 내가 좋아요한 게시물 // 특정 게시물
        if (mode != 'ALL' && mode != 'USER' && mode != 'PLACE' && mode != 'LIKE' && mode != 'SINGLE') {
            res.json({ status: 'ERR_WRONG_PARAMS' });
            return;
        }
        ...
        if (mode == 'ALL' ) {
            ...
            query += " WHERE poTab.po_p_id = ? ORDER BY poTab.po_created_date DESC";
            params = [authUId, pId];
        ...
        res.json({ status: 'OK', result: postsList });
